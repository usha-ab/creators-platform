...
export const httpApi: RemoteApi = {
  async list() {
    const data = await json<{ documents: RemoteSummary[] }>(
      await fetch("/api/tts/documents", { cache: "no-store" })
    );
    return data.documents ?? [];
  },
  async upload(doc) {
    await json(
      await fetch("/api/tts/documents", {
        method: "POST",
  ...
  async patchProgress(id, progress) {
    await json(
      await fetch(`/api/tts/documents/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async remove(id) {
    const response = await fetch(`/api/tts/documents/${id}`, { method: "DELETE" });
  ...
  async fetchText(id) {
    const data = await json<{ document: { content: string } }>(
      await fetch(`/api/tts/documents/${id}`, { cache: "no-store" })
    );
